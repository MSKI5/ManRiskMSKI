  ...
  <div class="p-6 mx-auto space-y-6 max-w-7xl">
            ...
            <tr v-else v-for="item in assessments" :key="item.id" class="transition hover:bg-gray-50">
              ...
              <td class="p-4 text-center">
                <div class="inline-flex items-center justify-center gap-2">
                  <span class="px-2 py-1 text-xs font-bold bg-white border border-gray-200 rounded text-gray-600">{{ item.frequency }}</span>
                  <span class="text-gray-400">×</span>
                  <span class="px-2 py-1 text-xs font-bold bg-white border border-gray-200 rounded text-gray-600">{{ item.impact }}</span>
                </div>
              </td>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
      <div class="w-full max-w-lg p-6 overflow-hidden bg-white shadow-2xl rounded-2xl">
        <h3 class="text-xl font-bold text-gray-800 mb-1">Revisi Data Risiko</h3>
        <p class="text-sm text-gray-500 mb-6">{{ editForm.indicator_code }} - {{ editForm.indicator_name }}</p>
        ...
        <form @submit.prevent="saveEdit" class="space-y-5">
          ...
          <div class="p-4 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <div v-if="editForm.has_document" class="flex items-center justify-between p-2 mb-3 bg-blue-50 border border-blue-100 rounded-lg">
